...
const ProductsPage = () => {
  ...
  const filteredProducts = productData.filter(
    (product) => product.category === productItem.toLowerCase()
  );

  const dataSearch = filteredProducts.filter((product) =>
  ...
  return (
    <>
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-4 mt-10 sm:mt-3 justify-items-center sm:justify-items-center">
        {(dataSearch ? dataSearch : productData)
          .filter((product) => product.category === productItem)
          .map((product, index) => {
